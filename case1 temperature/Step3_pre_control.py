...
if __name__ == '__main__':
    ...
    for k in range(total_time - 1):
        ...
        for tau in range(k + 1, total_time):
            ...
            if k == 0:
                ...
                c_open_room3[k][tau] = room2_sigmas[str(k)][str(tau)]*c_open
    ...
    with open('data_pre_control/room2_test_predictions.json', 'w') as f:
        json.dump(room2_test_predictions, f)
    with open('data_pre_control/room3_test_predictions.json', 'w') as f:
        json.dump(room3_test_predictions, f)
    with open('data_pre_control/room2_ground.json', 'w') as f:
        json.dump(room2_ground, f)
    with open('data_pre_control/room3_ground.json', 'w') as f:
        json.dump(room3_ground, f)
    with open('data_pre_control/c_close_room2.json', 'w') as f:
        json.dump(c_close_room2, f)
    with open('data_pre_control/c_close_room3.json', 'w') as f:
        json.dump(c_close_room3, f)
    with open('data_pre_control/c_open_room2.json', 'w') as f:
        json.dump(c_open_room2, f)
    with open('data_pre_control/c_open_room3.json', 'w') as f:
