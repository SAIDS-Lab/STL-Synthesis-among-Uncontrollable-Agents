...
def PrintPaperFig(r1_trace_list_closed, r2_trace_list_closed, r3_trace_list_closed, r1_trace_list_open, r2_trace_list_open, r3_trace_list_open):
    ...
    for i in range(4):
        ...
        ax[i].plot(k_known, r2_trace_list[:k+1], linewidth=1, color = 'red', label = "Known states of $Y_1$" if i == 0 else None)
        ax[i].plot(k_known, r3_trace_list[:k+1], linewidth=1, color = 'blue', label = "Known states of $Y_2$" if i == 0 else None)
        ax[i].plot(k_unknown, r1_trace_list[k:], linestyle=':', linewidth=1, color = 'black', label = "predicted states of x" if i == 0 else None)
        ax[i].plot(k_unknown, r2_trace_list[k:], linestyle=':', linewidth=1, color = 'red', label = "Predicted states of $Y_1$" if i == 0 else None)
        ax[i].plot(k_unknown, r3_trace_list[k:], linestyle=':', linewidth=1, color = 'blue', label = "Predicted states of $Y_2$" if i == 0 else None)
    for i in range(4):
    ...
    # plt.show()
    plt.savefig("fig/case1_trace.pdf")
...
if __name__ == "__main__":
    ...
    with open("data_controlresults/r3_trace_list_closedloop.json") as f:
    ...
    with open("data_controlresults/r3_trace_list_openloop.json") as f:
        r3_trace_list_openloop = json.load(f)


    index  = 0
    PrintPaperFig(r1_trace_list_closedloop[str(index)], r2_trace_list_closedloop[str(index)], r3_trace_list_closedloop[str(index)], r1_trace_list_openloop[str(index)], r2_trace_list_openloop[str(index)], r3_trace_list_openloop[str(index)])
